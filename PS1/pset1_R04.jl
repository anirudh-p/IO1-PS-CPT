...
function model_crosselasticity(p, X, θ, ξ, ν)
    ...
    for m=1:100
        ...
        ϵ[3*m-2,3] = sum(α_i.*prob[m,:,3])/(1000)*(p[m,3]/s_1[m])
        ϵ[3*m-1,1] = sum(α_i.*prob[m,:,1])/(1000)*(p[m,1]/s_1[m])
        ϵ[3*m-1,3] = sum(α_i.*prob[m,:,3])/(1000)*(p[m,3]/s_1[m])
        ϵ[3*m,1] = sum(α_i.*prob[m,:,1])/(1000)*(p[m,1]/s_1[m])
        ϵ[3*m,2] = sum(α_i.*prob[m,:,2])/(1000)*(p[m,2]/s_1[m])
        q=1000*m
    end
